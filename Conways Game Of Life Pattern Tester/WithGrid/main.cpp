...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (!bSimulationStart)
		{
			for (auto &i : state)
			{
				i.shape.setSize(sf::Vector2f(nPixelSize - 1, nPixelSize - 1));
				if (!i.alive)
					i.shape.setFillColor(sf::Color::Color(128, 128, 128, 255));
			}
		}
		...
		if (bSimulationStart)
		{
			if (limit.asMilliseconds() > 10) //AKO HOCES SMANJITI BRZINU MICANJA ZA ODREDENI BROJ
			{
				limit -= limit;
